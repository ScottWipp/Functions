...
% STAT characterises the inputed distribution 'PDF' which is a single 
...
% numbers (such as [mean,std]) then the 3rd value will be 'NaN'. Output is
% always central value first, followed by the positive uncertainty, then
% the negative uncertainty (only one value if symetrical uncertainty). 
%
%
%   SYNTAX: 
...
elseif whatKind == 4 % mean +- from std of naturalLog(pdf)
    if sum(PDF<0) > 0
