...
if distpath > grid.lim(5)
    ...
    temp_flux  = bsxfun(@times,(p1-p2-p3),geoResponse); %kgU/m2 (flux assuming unity concentration)
    
    
%[geoResponse,flux,flux] = geoFlux(mass,distpath,s2.energy,s2.pee);
...
%(don't do this in loop as lat/lon doesnt change, only depth)
    ...
    %}
        ...
        rho = PREM(round(depth3/1000),3); %(kg/m3) Extract PREM densities
        [mass3,distpath3] = fluxGrid(lon3,lat3,lon_int3,lat_int3,depth3,d_int3,rho,SurfRadius,det); 
        ...
        temp_flux = temp_flux  + sum(unityFlux); % 'sum' sums flux for each energy
...
% it as long as 'aU'
flux_U238  = bsxfun(@times,flux,aU); %nu/m2/kg%.* s2.iso.avgd .* s2.iso.dc.U238 ./s2.iso.amu.U238 .*s2.iso.molar.U238; 
flux_Th232 = bsxfun(@times,flux,aTh); %.* s2.iso.avgd .* s2.iso.dc.Th232 ./s2.iso.amu.Th232  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 %}
    ...
    if s2.calcMantleLayered == true
        abund_sums_dm(:,1) = repmat(sum(mass1(1:end-1)),length(abund_mass),1); %(kg)
        abund_sums_em(:,1) = repmat(mass1(end),length(abund_mass),1); %(kg)
    else
        abund_sums_dm(:,1) = repmat(sum(mass1),length(abund_mass),1); %(kg)
        abund_sums_em(:,1) = zeros(length(abund_mass),1); %(kg)
    end     
