function [abund_stat,abund_sums,geoRespStat,flux_stat,flux_sums,temp_pressure,flux_count] = Huang13_cluster(n,iter,layer,s1,s2,cor,SurfRadius,detector,P)
...
if distpath > grid.lim(5)
    ...
    geoResponse = median(mass)./(4*pi*distpath.^2); 
    ...
    % - Record Flux vs Distance (for plotting) -
            ...
            dist_count(index,:) = dist_count(index,:) + flux; 
    ...
    num2(num2<100)=100; % <100 m size doesnt improve signal but takes longer
        ...
        if num2*1.333 <= num %i.e. is voxel larger than desired size.  If the new size*1.33 isnt larger than it isnt worth gridding
            ...
            x = unique(index);
            for i = 1:length(x) %loop through unique distances and sum
            dist_count(x(i),:) = dist_count(x(i),:) + sum(unityFlux(index==x(i),:)); 
            end
...
% -- Record Flux vs Distance (for plotting) --
dist_count_U238 = bsxfun(@times,dist_count,median(aU)); % only do median (otherwise 50x19xiterations)
